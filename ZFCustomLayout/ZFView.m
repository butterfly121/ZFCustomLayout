...
-(void)setupTopView
{
    _topView = [[UIView alloc] initWithFrame:CGRectMake(0, 20, WINDOW_W, 26)];
...
-(void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    if (isIntersect) {
        //更换占位按钮与相交按钮位置
        [self.selectedbtnArray exchangeObjectAtIndex:[self.selectedbtnArray indexOfObject:object] withObjectAtIndex:[self.selectedbtnArray indexOfObject:_blankBtn]];
        //刷新
...
-(void)button:(ZFButton *)btn didPan:(UIGestureRecognizer *)gesture
{
    ...
    switch (gesture.state) {
        case UIGestureRecognizerStateBegan:{
            //占位按钮替换掉拖动按钮
            [self.selectedbtnArray replaceObjectAtIndex:[self.selectedbtnArray indexOfObject:gesture.view] withObject:_blankBtn];
        ...
        case UIGestureRecognizerStateEnded:{
            //拖动结束,用拖动按钮将空白按钮替换
            [self.selectedbtnArray replaceObjectAtIndex:[self.selectedbtnArray indexOfObject:_blankBtn] withObject:gesture.view];
            [self reloadData:self.selectedbtnArray];
        }
            break;
        default:
